"""Scoring engine with GeckoTerminal discovery and DEX Screener enrichment."""
...
class TokenScoringService:
    ...
    def _eligible_snapshots(
        ...
        for snapshot in snapshots:
            ...
            if (
                ...
                and snapshot.age_minutes > max_age_minutes
            ):
                continue
    ...
    def _fetch_live_snapshots(self, strategy: Strategy) -> list[TokenSnapshot]:
        ...
        try:
            return self._enrich_with_dexscreener(snapshots)
    ...
    def _snapshot_from_dex_pair(
        ...
        return TokenSnapshot(
            ...
            symbol=symbol,
            name=name,
            first_seen_at=self._parse_unix_ms(pair.get('pairCreatedAt')),
            liquidity_usd=liquidity_usd,
            volume_15m_usd=volume_15m_usd,
    ...
    def _parse_unix_ms(self, raw_value: Any) -> datetime:
        try:
